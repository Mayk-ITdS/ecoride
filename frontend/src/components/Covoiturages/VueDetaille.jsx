...
export default function CovoiturageDetail() {
  ...
  const moyenneAvis =
    trajet.avis && trajet.avis.length > 0
      ? (
          trajet.avis.reduce((acc, a) => acc + a.note, 0) / trajet.avis.length
        ).toFixed(1)
      : "Pas encore d'avis";
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-green-100 p-6">
      <div
        key={trajet.id}
        className="max-w-3xl mx-auto bg-white shadow rounded-2xl p-6 space-y-6"
      >
        <div className="flex items-center gap-4">
          <img
            // src={trajets.chauffeur.avatar} a faire
        ...
        <div className="p-4 bg-gray-50 rounded-lg">
          ...
          <p>
            Type:{" "}
            {trajet.voiture.eco ? "Électrique / Éco" : "Thermique classique"}
          </p>
        </div>
        <div className="p-4 bg-gray-50 rounded-lg">
          ...
          <ul className="list-disc pl-5 text-gray-600">
            <li>
              🚭 {trajet.chauffeur.preferences.fumeur ? "Fumeur" : "Non fumeur"}
            </li>
            <li>
              🐶{" "}
              {trajet.chauffeur.preferences.animaux
                ? "Animaux acceptés"
                : "Pas d'animaux"}
            </li>
            <li>
              🎵{" "}
              {trajet.chauffeur.preferences.musique
                ? "Musique OK"
                : "Pas de musique"}
            </li>
          </ul>
