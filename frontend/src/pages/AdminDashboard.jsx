import { useEffect, useMemo, useState, useCallback, useRef } from 'react'
import useAxiosWithAuth from '../hooks/useAxiosWithAuth'
import {
  AreaChart,
  Area,
  XAxis,
  YAxis,
  Tooltip,
  ResponsiveContainer,
  CartesianGrid,
  Legend,
} from 'recharts'
import CreateEmployeeModal from '../components/AdminDashboard/CreateEmployeeModal.jsx'

export default function AdminDashboard() {
  const api = useAxiosWithAuth()

  const [metrics, setMetrics] = useState(null)
  const [users, setUsers] = useState([])
  const [total, setTotal] = useState(0)
  const [page, setPage] = useState(1)
  const [limit] = useState(10)
  const [q, setQ] = useState('')
  const [loading, setLoading] = useState(true)
  const [err, setErr] = useState(null)
  const [showCreate, setShowCreate] = useState(false)

  const qRef = useRef(q)
  useEffect(() => {
    qRef.current = q
  }, [q])

  const loadAll = useCallback(async () => {
    try {
      setLoading(true)
      const [{ data: m }, { data: u }] = await Promise.all([
        api.get('/admin/metrics'),
        api.get('/admin/users', { params: { page, limit, q: qRef.current } }),
      ])
      setMetrics(m ?? {})
      setUsers(u?.items ?? [])
      setTotal(u?.total ?? 0)
      setErr(null)
    } catch (e) {
      setErr(e?.response?.data?.error || 'Erreur de chargement')
    } finally {
      setLoading(false)
    }
  }, [api, page, limit])

  useEffect(() => {
    const id = setTimeout(() => {
      setPage(1)
      loadAll()
    }, 300)
    return () => clearTimeout(id)
  }, [q, loadAll])

  useEffect(() => {
    loadAll()
  }, [page, limit, loadAll])

  const pages = useMemo(
    () => Math.max(Math.ceil(total / limit), 1),
    [total, limit],
  )

  const suspend = async (u, suspended) => {
    const prev = users
    try {
      setUsers((xs) =>
        xs.map((x) =>
          x.id_user === u.id_user ? { ...x, is_suspended: suspended } : x,
        ),
      )
      await api.patch(`/admin/users/${u.id_user}/suspend`, { suspended })
    } catch {
      setUsers(prev)
    }
  }
  const stop = (e) => {
    e.preventDefault()
    e.stopPropagation()
  }

  const prevPage = (e) => {
    stop(e)
    setPage((p) => Math.max(1, p - 1))
  }
  const nextPage = (e) => {
    stop(e)
    setPage((p) => Math.min(pages, p + 1))
  }
  if (loading)
    return <div className="p-6 bg-white rounded-2xl shadow">Chargement…</div>
  if (err)
    return (
      <div className="p-6 bg-white rounded-2xl shadow text-red-600">{err}</div>
    )

  const tripsPerDay = [...(metrics?.tripsPerDay ?? [])].reverse()
  const creditsPerDay = [...(metrics?.creditsPerDay ?? [])].reverse()

  return (
    <div className="space-y-8">
      <div className="grid md:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="text-sm text-gray-500">Crédits gagnés (total)</div>
          <div className="text-3xl font-semibold">
            {metrics?.totalCredits ?? 0}
          </div>
        </div>
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="text-sm text-gray-500">Jours (trajets)</div>
          <div className="text-3xl font-semibold">{tripsPerDay.length}</div>
        </div>
        <div className="bg-white rounded-2xl shadow p-4">
          <div className="text-sm text-gray-500">Jours (crédits)</div>
          <div className="text-3xl font-semibold">{creditsPerDay.length}</div>
        </div>
      </div>
      <div className="bg-white rounded-2xl shadow p-4">
        <h3 className="text-lg font-semibold mb-3">Covoiturages par jour</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={tripsPerDay}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="day" />
              <YAxis allowDecimals={false} />
              <Tooltip />
              <Legend />
              <Area type="monotone" dataKey="trips" name="trajets" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      <div className="bg-white rounded-2xl shadow p-4">
        <h3 className="text-lg font-semibold mb-3">Crédits gagnés par jour</h3>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={creditsPerDay}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="day" />
              <YAxis allowDecimals={false} />
              <Tooltip />
              <Legend />
              <Area type="monotone" dataKey="credits" name="crédits" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold">Utilisateurs</h3>
        <button
          onClick={() => setShowCreate(true)}
          className="px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700"
        >
          Créer un employé
        </button>
      </div>
      <div className="bg-white rounded-2xl shadow p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-lg font-semibold">Utilisateurs</h3>
          <input
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder="Rechercher pseudo/email…"
            className="border rounded-lg px-3 py-2 text-sm"
          />
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="text-sm text-gray-600 border-b">
                <th className="py-2 pr-3">ID</th>
                <th className="py-2 pr-3">Pseudo</th>
                <th className="py-2 pr-3">Email</th>
                <th className="py-2 pr-3">Rôles</th>
                <th className="py-2 pr-3">Statut</th>
                <th className="py-2 pr-3">Actions</th>
              </tr>
            </thead>
            <tbody>
              {users.map((u) => (
                <tr key={u.id_user} className="border-b">
                  <td className="py-2 pr-3">{u.id_user}</td>
                  <td className="py-2 pr-3">{u.pseudo}</td>
                  <td className="py-2 pr-3">{u.email}</td>
                  <td className="py-2 pr-3 text-sm text-gray-600">
                    {(u.roles || []).join(', ')}
                  </td>
                  <td className="py-2 pr-3">
                    {u.is_suspended ? (
                      <span className="px-2 py-1 text-xs rounded bg-red-100 text-red-700">
                        suspendu
                      </span>
                    ) : (
                      <span className="px-2 py-1 text-xs rounded bg-emerald-100 text-emerald-700">
                        actif
                      </span>
                    )}
                  </td>
                  <td className="py-2 pr-3">
                    {u.is_suspended ? (
                      <button
                        onClick={() => suspend(u, false)}
                        className="px-3 py-1 text-sm rounded bg-emerald-600 text-white hover:bg-emerald-700"
                      >
                        Réactiver
                      </button>
                    ) : (
                      <button
                        onClick={() => suspend(u, true)}
                        className="px-3 py-1 text-sm rounded bg-red-600 text-white hover:bg-red-700"
                      >
                        Suspendre
                      </button>
                    )}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-gray-600">
            Page {page} / {pages} — {total} utilisateurs
          </div>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={prevPage}
              className="px-3 py-1 rounded bg-gray-100 hover:bg-gray-200"
              disabled={page <= 1}
            >
              Précédent
            </button>
            <button
              type="button"
              onClick={nextPage}
              className="px-3 py-1 rounded bg-gray-100 hover:bg-gray-200"
              disabled={page >= pages}
            >
              Suivant
            </button>
          </div>
        </div>
      </div>

      <CreateEmployeeModal
        open={showCreate}
        onClose={() => setShowCreate(false)}
        onCreated={loadAll}
      />
    </div>
  )
}
